...
public class ReactionRenderers {
    ...
    public void creation(Reactor reactor){
        ...
        for(BlockPos pos : points){
            if(reactor.getLevel().getBlockState(pos).isAir())
                ParticleScribe.drawParticleSphere(Objects.requireNonNull(reactor.getLevel()), ReactiveParticles.STARDUST_PARTICLE, pos, 0.5, 1.0, 1);
        }
    }
    ...
    public void acid_based(Reactor reactor) {
        ...
        if(level.random.nextFloat() < 0.1F)
            ParticleScribe.drawParticleReactionSurface(reactor.getLevel(), ReactiveParticles.ACID_BUBBLE_PARTICLE.getType(), reactor);
    }
    ...
    public void astral(Reactor reactor) {
        if(reactor.getPowerLevel(Powers.ASTRAL_POWER.get()) < reactor.getTotalPowerLevel())
            ParticleScribe.drawExactParticleRing(reactor.getLevel(), ReactiveParticles.STARDUST_PARTICLE.getType(), reactor.getPos(), 0.7, 1);
    }
