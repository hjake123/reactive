...
public class GatewayRenderer<T extends GatewayBlockEntity> implements BlockEntityRenderer<T> {
    ...
    public void render(@NotNull T gateway, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        ...
        this.renderVolume(gateway, matrix4f, bufferSource.getBuffer(RenderType.endGateway()), partialTick);
    }
    ...
    private void renderFace(Matrix4f pose, VertexConsumer consumer, float x0, float x1, float y0, float y1, float z0, float z1, float z2, float z3, float ul_distort, float dl_distort, float ur_distort, float dr_distort) {
        consumer.vertex(pose, adjust(x0, dl_distort), adjust(y0,  dl_distort), adjust(z0, dl_distort)).endVertex();
        consumer.vertex(pose, adjust(x1,  dr_distort), adjust(y0,  dr_distort), adjust(z1, dr_distort)).endVertex();
        consumer.vertex(pose, adjust(x1,  ur_distort), adjust(y1,  ur_distort), adjust(z2, ur_distort)).endVertex();
        consumer.vertex(pose, adjust(x0,  ul_distort), adjust(y1,  ul_distort), adjust(z3, ul_distort)).endVertex();
    }
