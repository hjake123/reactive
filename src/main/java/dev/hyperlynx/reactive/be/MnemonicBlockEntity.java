...
public class MnemonicBlockEntity extends BlockEntity {
    ...
    public MnemonicBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(Registration.MNEMONIC_BULB_BE_TYPE.get(), pPos, pBlockState);
    }
    ...
    public void performTick(Level level, BlockPos pos, BlockState state){
        ...
        switch(status){
            ...
            case RECORDING -> {
                if (!memory.isEmpty() && memory.get(memory.size() - 1).signal == input) {
                    memory.get(memory.size() - 1).increment();
                } else {
                ...
                if(memory.get(memory.size() - 1).duration > MAX_SIGNAL_DURATION){
                    if(input == 0) {
